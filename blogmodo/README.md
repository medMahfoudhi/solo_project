...
- [ ] Refactor the **Feed** component to dynamically render the sample data in `sample_data.js`. Each object in sample_data represents one blog post.
...
- [ ] In the Express server `server/index.js`, complete the request handler that will respond to `GET` requests to `/api/blogs` with JSON of the blog posts stored in the database. You should use the Mongoose model exported by `Blog.js` to fetch all of the blogs from the database./or if you are working with MYSQL create your model in database-mysql/index.js (You know the drill)
- [ ] Make an AJAX request / Or axios from your client to the `/api/blogs` route of your server when the App (or Feed) component gets loaded. Replace the sample data in your client with data obtained from your server.
...
**Implement this user story by doing the following:**
  ...
  - [ ] Refactor the **Post** component to render one blog post.
